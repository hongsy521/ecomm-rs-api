...
class SellerProductControllerTest {
    ...
    @Test
    @WithMockCustomUser(id = 1L, roles = {"SELLER"})
    ...
    void testRegisterProduct() throws Exception {
        ...
        // given 3
        given(sellerProductService.registerProduct(any(Long.class),any(ProductRequestDto.class))).willReturn(
            mockResponseDto);

        // when
        mockMvc.perform(post("/api/seller/product/register").contentType(MediaType.APPLICATION_JSON).content(
                objectMapper.writeValueAsString(requestDto))).andExpect(status().isOk())
            .andExpect(jsonPath("$.data.colorGroup").value("검정색"));

        // then
        verify(sellerProductService, times(1)).registerProduct(any(Long.class),any(ProductRequestDto.class));
    }
